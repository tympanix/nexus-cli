...
- `deps.toml` - Dependency manifest (version-controlled)
- `deps-lock.toml` - Lock file with resolved checksums (version-controlled)
...
The `deps.toml` file defines your project's dependencies. It uses TOML format with a `[defaults]` section and `[deps.<name>]` sections for each dependency.
...
recursive = true                        # optional, download folder recursively
```
...
The `deps-lock.toml` file contains resolved file paths and their checksums. It is generated by `nexuscli-go deps lock` and should be committed to version control alongside `deps.toml`.
