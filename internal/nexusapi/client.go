...
func (c *Client) SearchAssetsForCompletion(repository, pathPrefix string) ([]string, error) {
	...
	if pathPrefix != "" {
		...
		searchPath := pathpkg.Join("/", pathPrefix)
...
// SearchAssetsByName searches for assets by name pattern in a repository
// Returns all assets matching the name parameter (supports wildcards as determined by Nexus)
func (c *Client) SearchAssetsByName(repository, name string) ([]Asset, error) {
	var assets []Asset
	continuationToken := ""
	...
	for {
		...
		query := baseURL.Query()
		query.Set("repository", repository)
		// Ensure name starts with / as required by Nexus API
		searchName := name
