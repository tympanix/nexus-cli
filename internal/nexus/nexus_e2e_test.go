...
func TestEndToEndUploadDownload(t *testing.T) {
	...
	repoName := "test-repo"
	if err := createRawRepository(cfg, repoName); err != nil {
	...
	err = uploadFiles(testDir, repoName, "test-folder", cfg, uploadOpts)
	if err != nil {
	...
	success := downloadFolder(uploadPath, downloadDir, cfg, downloadOpts)
	if !success {
...
func createRawRepository(cfg *config.Config, repoName string) error {
	...
	req, err := http.NewRequest("POST", cfg.NexusURL+"/service/rest/v1/repositories/raw/hosted", bytes.NewBuffer(jsonData))
	if err != nil {
