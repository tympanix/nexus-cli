...
func listAssets(repository, src string, cfg *config.Config) ([]Asset, error) {
	var assets []Asset
	continuationToken := ""
	for {
		baseURL, err := url.Parse(cfg.NexusURL)
		if err != nil {
...
func downloadAssetUnified(asset Asset, destDir string, wg *sync.WaitGroup, errCh chan error, bar *progressbar.ProgressBar, skipCh chan bool, cfg *config.Config, opts *DownloadOptions) {
	defer wg.Done()
	...
	resp.SetBasicAuth(cfg.Username, cfg.Password)
...
func downloadFolder(srcArg, destDir string, cfg *config.Config, opts *DownloadOptions) bool {
	...
	assets, err := listAssets(repository, src, cfg)
	if err != nil {
	...
	for _, asset := range assets {
		wg.Add(1)
		go func(asset Asset) {
			downloadAssetUnified(asset, destDir, &wg, errCh, bar, skipCh, cfg, opts)
		}(asset)
	}
	wg.Wait()
	close(errCh)
...
// Download downloads files from Nexus repository to dest directory
func Download(src, dest string, cfg *config.Config, opts *DownloadOptions) {
	success := downloadFolder(src, dest, cfg, opts)
	if !success {
		os.Exit(66)
	}
}
