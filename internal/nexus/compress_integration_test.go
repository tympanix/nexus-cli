...
// TestCompressedUploadZstd tests uploading files as a zstd compressed archive
func TestCompressedUploadZstd(t *testing.T) {
// Create test files
testDir, err := os.MkdirTemp("", "test-compress-upload-zstd-*")
if err != nil {
t.Fatalf("Failed to create test directory: %v", err)
}
defer os.RemoveAll(testDir)
...
expectedName := "test-repo-test-folder.tar.zst"
if receivedArchiveName != expectedName {
t.Errorf("Expected archive name %q, got %q", expectedName, receivedArchiveName)
}
}

// TestCompressedDownloadZstd tests downloading and extracting a zstd compressed archive
func TestCompressedDownloadZstd(t *testing.T) {
// Create test files
srcDir, err := os.MkdirTemp("", "test-compress-dl-zstd-src-*")
if err != nil {
t.Fatalf("Failed to create source directory: %v", err)
}
defer os.RemoveAll(srcDir)
...
// Create archive file
archiveFile, err := os.CreateTemp("", "test-archive-zstd-*.tar.zst")
if err != nil {
t.Fatalf("Failed to create archive file: %v", err)
}
archivePath := archiveFile.Name()
...
if err := CreateTarZstd(srcDir, archiveFile); err != nil {
t.Fatalf("Failed to create archive: %v", err)
}
archiveFile.Close()
...
// Create mock server
server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
if r.Method == "GET" && strings.Contains(r.URL.RawQuery, "repository=test-repo") {
// Return assets list with archive
...
// Create download directory
destDir, err := os.MkdirTemp("", "test-compress-dl-zstd-dest-*")
if err != nil {
t.Fatalf("Failed to create destination directory: %v", err)
}
defer os.RemoveAll(destDir)
